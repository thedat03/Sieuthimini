    ...
    deleteProduct(product: Product): void {
      ...
      if (confirmation) {
        this.productService.deleteProduct(product.id).subscribe({
          next: (response: any) => {
            alert('Product deleted successfully!'); // Hiển thị thông báo thành công
            this.products = this.products.filter(p => p.id !== product.id); // Cập nhật danh sách sản phẩm
          },
          error: (error: any) => {
            console.error('Error deleting product:', error);
            // Chuyển đổi đối tượng lỗi thành chuỗi có ý nghĩa
            const errorMessage = error.error?.message || 'An error occurred while deleting the product.';
